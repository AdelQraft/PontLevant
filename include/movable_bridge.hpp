...
class MovableBridge
{
    static_assert(std::is_base_of<StepperDriver::IStepperDriver, StepperDriverT1>::value,
                  "StepperDriverT1 must inherit from StepperDriver::IStepperDriver");
    static_assert(std::is_base_of<StepperDriver::IStepperDriver, StepperDriverT2>::value,
                  "StepperDriverT2 must inherit from StepperDriver::IStepperDriver");
...
#define callSteppersMethod(method, ...) \
    ...
    void openBoth()
    {
        ...
        uint_fast32_t halfSpeedDelay = static_cast<uint_fast32_t>(1e6) / stepperDriver1.speed / 2;
        ...
        do
        {
            ...
            cptr++;
        } while (stepperDriver1.targetStep != stepperDriver1.currentStep &&
                 stepperDriver2.targetStep != stepperDriver2.currentStep);
    }
